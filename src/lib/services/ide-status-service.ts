...
export class IDEStatusService {
    ...
    private static formatStatusForDisplay(status: IDEStatus): string {
        ...
        const avgProcessingTime = status.totalEventProcessingTime && status.nbrOfEventsProcessed && status.nbrOfEventsProcessed > 0 ? (status.totalEventProcessingTime / status.nbrOfEventsProcessed) : 'N/A';
